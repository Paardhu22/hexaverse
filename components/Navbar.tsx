...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 glass border-b border-white/5 shadow-2xl">
            ...
            <div className="ml-10 flex items-baseline space-x-2">
              {links.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-gray-400 hover:text-white hover:bg-white/5 transition-all border border-transparent hover:border-white/10"
                >
                  {link.icon}
                  {link.name}
                </Link>
      ...
      {/* Mobile Drawer */}
      <div 
        className={`md:hidden absolute w-full left-0 transition-all duration-300 ease-in-out bg-black/95 backdrop-blur-2xl border-b border-white/10 shadow-2xl ${
